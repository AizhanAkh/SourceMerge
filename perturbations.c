...
int perturb_init(
  ...
  if (ppt->dmeff_Vrms_dynamic == _TRUE_ || ppt->dmeff_Vfull_dynamic == _TRUE_){
    class_alloc(ppt->Vrms_k_dmeff_dynamic,
                ...
                ppt->error_message);
    class_alloc(ppt->Vflow_k_dmeff_dynamic,
                ppt->k_size[ppt->index_md_scalars] * pba->bt_size * sizeof(double),
                ppt->error_message);
    class_alloc(ppt->V2integrand_k_dmeff_dynamic,
                ppt->k_size[ppt->index_md_scalars] * pba->bt_size * sizeof(double),
                ppt->error_message);
  }
  ...
  num_threads(number_of_threads)
    ...
    for (index_ic = 0; index_ic < ppt->ic_size[index_md]; index_ic++) {
      ...
      if(_scalars_ && index_ic==ppt->index_ic_ad && (ppt->dmeff_Vrms_dynamic==_TRUE_ || ppt->dmeff_Vfull_dynamic==_TRUE_)){
        index_dmeff_niter_max = ppt->dmeff_niter_dynamic_max;
      }

      while(pba->index_dmeff_niter < index_dmeff_niter_max){
        /* initialize/fill Vrms and Vflow */
        if(ppt->dmeff_Vrms_dynamic==_TRUE_ || ppt->dmeff_Vfull_dynamic==_TRUE_){
          class_call(perturb_dmeff_Vrms_dynamic_fill(ppr,pba,pth,ppt,index_md,index_ic),
                     ppt->error_message,
                     ppt->error_message);
           //VP: add a convergence test to stop iterating.
           index_tau = 0; //we take the highest z to check for convergence.
           convergence = fabs((pba->background_table[index_tau*pba->bg_size+pba->index_bg_Vrms]-Vrms_old)/Vrms_old);
            if(convergence < ppt->Vrms_convergence_tol && pba->index_dmeff_niter>0){
              if (ppt->perturbations_verbose > 0)
                printf(" -> Achieved convergence %e in Vrel for dmeff for %d iterations.\n",convergence,pba->index_dmeff_niter + 1);
              pba->index_dmeff_niter = index_dmeff_niter_max-1; // VP: we have achieved convergence.
            }
            else {
              if (ppt->perturbations_verbose > 0)
                printf(" -> Current convergence %e in Vrel for dmeff for %d iterations.\n",convergence,pba->index_dmeff_niter + 1);
              Vrms_old = pba->background_table[index_tau*pba->bg_size+pba->index_bg_Vrms];
            }
        }
...
#pragma omp flush(abort)
      ...
      if(index_dmeff_niter_max > 1 && convergence >= ppt->Vrms_convergence_tol){
        printf(" -> Vrel convergence %e failed to achieve desired %e\n",convergence,ppt->Vrms_convergence_tol);
      }
...
int perturb_free(
  ...
  if (ppt->has_perturbations == _TRUE_) {
    ...
    if (ppt->dmeff_Vrms_dynamic == _TRUE_ || ppt->dmeff_Vfull_dynamic == _TRUE_){
      ...
      free(ppt->Vflow_k_dmeff_dynamic);
...
int perturb_vector_init(
  ...
  if (_scalars_) {
    ...
    if(ppt->gauge == synchronous && pba->has_cdm == _FALSE_){
      //we do no declare theta_dmeff
    }
    else{
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      if (pba->has_dmeff == _TRUE_) {
        ...
        if(pba->has_cdm == _TRUE_ && ppt->gauge == synchronous){
          // ppw->pv->y[ppw->pv->index_pt_theta_dmeff] = 0; /* dmeff velocity */
          ppw->pv->y[ppw->pv->index_pt_theta_dmeff] = ppw->pv->y[ppw->pv->index_pt_theta_g]; /* dmeff velocity */
        }
        else{
          /* dmeff velocity velocity vanishes initially in the synchronous gauge */
        }
 ...
 *
 * @param tau                      Input: conformal time
 * @param theta_b                  Input: theta for baryons
 * @param theta_dmeff              Input: theta for dmeff
 * @param parameters_and_workspace Input: same as that used in perturb_derivs()
 * @param Vrms                     Output: RMS velocity
 * @param Vflow                    Output: Vflow velocity
 */

int perturb_dmeff_Vrms_dynamic(double tau,
                               double theta_b,
                               double theta_dmeff,
                               struct perturb_parameters_and_workspace * pppaw,
                               double * Vrms,
  ...
  int index_k, index_md, index_ic, index_tau;
  double k;
  ...
  // check ICs
  class_test(index_ic != ppt->index_ic_ad,
             ppt->error_message,
             "Only adiabatic ICs are coded in dmeff dynamic Vrms and Vflow calculation.");

  // check mode
  class_test(index_md != ppt->index_md_scalars,
             ppt->error_message,
             "Why are non-scalar modes calling dmeff dynamic Vrms and Vflow calculation?");

  // determine index_tau
  if(pppaw->index_vrms_tau_current==0){
    // this is the first call to solver for this k
    index_tau = 0;
    while( (pba->tau_table[index_tau] < tau) && (index_tau < pba->bt_size) ) { index_tau++; }
    class_test(index_tau==pba->bt_size,
               ppt->error_message,
               "Could not find correct tau index to compute Vrms.");
    index_tau--;
    pppaw->index_vrms_tau_lower = index_tau;
    pppaw->index_vrms_tau_current = index_tau;

    // fill in values for index_tau < pppaw->index_vrms_tau_lower
    for(index_tau=0; index_tau < pppaw->index_vrms_tau_lower; index_tau++){
      ppt->V2integrand_k_dmeff_dynamic[index_tau * ppt->k_size[index_md] + index_k]
        = pow( (theta_dmeff - theta_b)/k, 2. ) / k * ppt->A_s * pow(k/ppt->k_pivot,ppt->n_s - 1.);
    }
    index_tau = pppaw->index_vrms_tau_lower;
  } else{
    index_tau = pppaw->index_vrms_tau_current;
    while( (pba->tau_table[index_tau] < tau) && (index_tau < pba->bt_size) ) { index_tau++; }
    index_tau--;

    if(index_tau > pppaw->index_vrms_tau_current + 1){
      // if integration step is so large that it covers multiple values of index_tau, fill in skipped entries
      while( pppaw->index_vrms_tau_current < index_tau ){
        pppaw->index_vrms_tau_current++;
        ppt->V2integrand_k_dmeff_dynamic[pppaw->index_vrms_tau_current * ppt->k_size[index_md] + index_k]
          = ppt->V2integrand_k_dmeff_dynamic[(pppaw->index_vrms_tau_current - 1) * ppt->k_size[index_md] + index_k];
      }
    }
    // in either case, we end up with pppaw->index_vrms_tau_current == index_tau
  }

  // cumulatively add Vrms (for a given index_tau) from large modes to small modes
  // to get Vrms(k), obtained by integrating from k (mode under consideration) to all k available
  ppt->V2integrand_k_dmeff_dynamic[index_tau * ppt->k_size[index_md] + index_k] = pow( (theta_dmeff - theta_b)/k, 2. ) / k * ppt->A_s * pow(k/ppt->k_pivot,ppt->n_s - 1.);

  // assign Vrms and Vflow correctly
  if(ppt->dmeff_Vrms_dynamic == _TRUE_){
    *Vrms  = ppt->Vrms_k_dmeff_dynamic[index_tau * ppt->k_size[index_md] + index_k];
    *Vflow = ppt->Vflow_k_dmeff_dynamic[index_tau * ppt->k_size[index_md] + index_k];
  } else{ // ppt->dmeff_Vfull_dynamic == _TRUE_
    *Vrms  = pba->background_table[index_tau*pba->bg_size+pba->index_bg_Vrms];
    *Vflow = 0.0;
  }
...
/**
 * Fill dynamic Vrms, Vflow for dmeff.
 *
 * @param ppr         Input: pointer to precision structure
 * @param pba         Input: pointer to background structure
 * @param pth         Input: pointer to the thermodynamics structure
 * @param ppt         Input: pointer to the perturbation structure
 * @param index_md    Input: index of mode under consideration (scalar/.../tensor)
 * @param index_ic    Input: index of initial condition under consideration
 */

int perturb_dmeff_Vrms_dynamic_fill(struct precision * ppr,
                                    struct background * pba,
                                    struct thermo * pth,
                                    struct perturbs * ppt,
                                    int index_md,
  ...
  int index, index_k, index_tau, index_tau_max;
  double delta_k, value, value_max, z;
  double *d2bg_dtau2_table;

  /* If this is the first call, initialize properly, as ppt->V2integrand_k_dmeff_dynamic has not yet been filled. */
  if(pba->index_dmeff_niter==0){
    for(index_tau=0; index_tau<pba->bt_size; index_tau++){
      for(index_k=0; index_k<ppt->k_size[index_md]; index_k++){
        index = index_tau * ppt->k_size[index_md] + index_k;
        ppt->Vrms_k_dmeff_dynamic[index] = pba->background_table[index_tau*pba->bg_size+pba->index_bg_Vrms];
        ppt->Vflow_k_dmeff_dynamic[index] = 0.0;
      }
    }
    return _SUCCESS_;
  }

  /* Compute Vflow and full Vrms. Save as V^2 in units of c (convert in next 'for' block). */
  value_max = 0.0;
  index_tau_max = 0;
  for(index_tau=0; index_tau<pba->bt_size; index_tau++){
    // perform trapezoidal integration over k
    value = 0.0;
    ppt->Vflow_k_dmeff_dynamic[index_tau * ppt->k_size[index_md] + 0] = 0.0;
    for(index_k=0; index_k<(ppt->k_size[index_md]-1); index_k++){
      delta_k = ppt->k[index_md][index_k+1] - ppt->k[index_md][index_k];
      value += (ppt->V2integrand_k_dmeff_dynamic[index_tau * ppt->k_size[index_md] + (index_k+1)] +
                ppt->V2integrand_k_dmeff_dynamic[index_tau * ppt->k_size[index_md] + index_k]) * delta_k/2.;
      ppt->Vflow_k_dmeff_dynamic[index_tau * ppt->k_size[index_md] + index_k+1] = value;
    }
    // fill in background table with full Vrms (integrated over all k)
    pba->background_table[index_tau*pba->bg_size+pba->index_bg_Vrms] = value;
    // above z_rec, find largest value of Vrms
    z = 1./pba->background_table[index_tau*pba->bg_size+pba->index_bg_a] - 1.;
    if(z > pth->z_rec && value > value_max){
      value_max = value;
      index_tau_max = index_tau;
    }
  }

  /* Modes are computed only up to a certain value of k. The coherence region
     between DM and baryons shifts to higher k at higher values of z (e.g., see
     Tseliakhovich and Hirata, 1005.2416), so the calculation of the full Vrms
     drops at some high z (above recombination) when it should stay constant.
     Correct this issue by enforcing the full Vrms stays constant. Note that the
     calculation of Vflow is fine.
     With the corrected full Vrms and Vflow, compute the k-dependent Vrms. */
  for(index_tau=0; index_tau<pba->bt_size; index_tau++){
    // correct full Vrms
    if(index_tau < index_tau_max){
      pba->background_table[index_tau*pba->bg_size+pba->index_bg_Vrms] = value_max;
    }

    // find Vrms from full Vrms and Vflow
    for(index_k=0; index_k<ppt->k_size[index_md]; index_k++){
      index = index_tau * ppt->k_size[index_md] + index_k;
      ppt->Vrms_k_dmeff_dynamic[index] = pba->background_table[index_tau*pba->bg_size+pba->index_bg_Vrms] - ppt->Vflow_k_dmeff_dynamic[index];
      if(ppt->Vrms_k_dmeff_dynamic[index] < 0.0) {
        ppt->Vrms_k_dmeff_dynamic[index] = 0.0; // in case of small rounding errors
      }

      // take square root and convert to units of m/s
      ppt->Vrms_k_dmeff_dynamic[index] = sqrt(ppt->Vrms_k_dmeff_dynamic[index])*_c_;
      ppt->Vflow_k_dmeff_dynamic[index] = sqrt(ppt->Vflow_k_dmeff_dynamic[index])*_c_;
    }
    pba->background_table[index_tau*pba->bg_size+pba->index_bg_Vrms]
      = sqrt(pba->background_table[index_tau*pba->bg_size+pba->index_bg_Vrms])*_c_;
  }

  /* After saving to background table, must recompute 2nd derivative for spline interpolation. */
  ...
  /* With the full Vrms from the previous iteration, recompute thermodynamics. */
  if (thermodynamics_free(pth) == _FAILURE_) {
    printf("\n\nError in thermodynamics_free when iterating \n=>%s\n",ppt->error_message);
    return _FAILURE_;
  }

  if (thermodynamics_init(ppr,pba,pth) == _FAILURE_) {
    printf("\n\nError in thermodynamics_init when iterating \n=>%s\n",ppt->error_message);
    return _FAILURE_;
  }
...
int perturb_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    if (pba->has_dmeff == _TRUE_) {
      ...
      /** ---> newtonian gauge: dmeff density and velocity */

      if (ppt->gauge == newtonian) {
        dy[pv->index_pt_delta_dmeff] = -(theta_dmeff+metric_continuity); /* dmeff density */

        dy[pv->index_pt_theta_dmeff] = - a_prime_over_a*theta_dmeff + metric_euler; /* dmeff velocity */
      }

      /** ---> synchronous gauge: dmeff density and velocity */
      /* Be sure to use dmeff speed of sound, temperature, or rates from thermo and not background */

      if (ppt->gauge == synchronous) {
        dy[pv->index_pt_delta_dmeff] = -(theta_dmeff+metric_continuity); /* dmeff density */

        // determine which Vrms to use and update modified momentum-transfer rate
        if(ppt->dmeff_Vrms_dynamic == _TRUE_ || ppt->dmeff_Vfull_dynamic == _TRUE_) {
          class_call(perturb_dmeff_Vrms_dynamic(tau,theta_b,theta_dmeff,pppaw,&Vrms_dmeff,&Vflow_dmeff),
                     ppt->error_message,
                     ppt->error_message);
          class_call(thermodynamics_dmeff_rate(pba,pth,pvecback,pvecthermo,Vrms_dmeff,Vflow_dmeff),
                     pth->error_message,
                     ppt->error_message);
        }
        ...
        if(pba->has_cdm == _TRUE_){
          // if true, synchronous gauge defined in CDM fluid
          // if false, synchronous gauge defined in dmeff fluid (theta_dmeff set to 0)
        ...
        class_stop(error_message,"dm-b interactions are not coded in the newtonian gauge!");
      }
    ...
    if (ppw->approx[ppw->index_ap_tca] == (int)tca_off) {
      ...
      if(pba->has_dmeff==_TRUE_){
        tau_c_dot =  2 * a_prime_over_a / rate_dmeff; // -dkappa/kappa^2
        tau_c = 1/rate_dmeff;
        beta += beta_dmeff;
      }
      ...
      dy[pv->index_pt_theta_b] =
        ...
        +metric_euler;
      if(pba->has_dmeff == _TRUE_){
        dy[pv->index_pt_theta_b] += (R*beta*(a_prime_over_a-tau_c_dot/tau_c)*(theta_dmeff-theta_b) + R_dmeff/tau_c*(theta_dmeff-theta_b) +R*beta*dy[pv->index_pt_theta_dmeff])/(1.+R+beta*R);
