 ...
 * @param pvecback   Input: vector of background quantities (used only in case z>z_initial for getting ddkappa and dddkappa; in that case, should be already allocated and filled, with format long_info or larger; in other cases, will be ignored)
...
int thermodynamics_at_z(
  ...
  if (z >= pth->z_table[pth->tt_size-1]) {
    ...
    pvecthermo[pth->index_th_xH] = x0; // assume all He is ionized at high z
    ...
    if(pba->has_dmeff == _TRUE_){
      ...
      pvecthermo[pth->index_th_dkappa_dmeff] = pvecback[pba->index_bg_dkappa_dmeff];
      pvecthermo[pth->index_th_dkappaT_dmeff] = pvecback[pba->index_bg_dkappaT_dmeff];
      pvecthermo[pth->index_th_orig_dkappa_dmeff] = pvecback[pba->index_bg_orig_dkappa_dmeff];
      pvecthermo[pth->index_th_orig_dkappaT_dmeff] = pvecback[pba->index_bg_orig_dkappaT_dmeff];
      pvecthermo[pth->index_th_heat_chi_dmeff] = pvecback[pba->index_bg_heat_chi_dmeff];
      pvecthermo[pth->index_th_heat_b_dmeff] = pvecback[pba->index_bg_heat_b_dmeff];
      pvecthermo[pth->index_th_cdmeff2] = pvecback[pba->index_bg_cdmeff2];
    }
...
int thermodynamics_init(
  ...
  /* structures for storing temporary information on dmeff */
  struct dmeff_thermo dmeff;
  ...
  /** - initialize dmeff (after setting Y_He) */
  if (pba->has_dmeff == _TRUE_){
    pdmeff=&dmeff;
    class_call(thermodynamics_dmeff_init(pba,pth,pdmeff),
               pth->error_message,
               pth->error_message);

    /* integrate temperature over same range in z as background quantities */
      class_call(thermodynamics_dmeff_temperature(ppr,pba,pth,pdmeff),
                 pth->error_message,
                 pth->error_message);
    ...
    /* save quantities */
    pth->z_dmeff_decoupling = pdmeff->zdec;
    pth->Tdmeff_initial = pdmeff->Tdmeff_initial;
  }

  /** - solve recombination and store values of \f$ z, x_e, d \kappa / d \tau, T_b, c_b^2 \f$ with thermodynamics_recombination() */

  class_call(thermodynamics_recombination(ppr,pba,pth,preco,pdmeff,pvecback),
             pth->error_message,
             pth->error_message);
  ...
  if (pth->reio_parametrization != reio_none) {
    class_call(thermodynamics_reionization(ppr,pba,pth,preco,preio,pdmeff,pvecback),
               pth->error_message,
               pth->error_message);
  }
  else {
  ...
  /* Free dmeff memory */
  if (pba->has_dmeff == _TRUE_){
    class_call(thermodynamics_dmeff_free(pdmeff),
               pth->error_message,
               pth->error_message);
  }
...
int thermodynamics_indices(
  ...
  if(pba->has_dmeff == _TRUE_){
    ...
    index++;
    pth->index_th_orig_dkappa_dmeff = index;
    index++;
    pth->index_th_orig_dkappaT_dmeff = index;
    index++;
    pth->index_th_heat_chi_dmeff = index;
    index++;
    pth->index_th_heat_b_dmeff = index;
    index++;
  ...
  /* dmeff quantities */
  if(pba->has_dmeff == _TRUE_){
    preco->index_re_Tdmeff = index;
    index++;
    preco->index_re_dkappa_dmeff = index;
    index++;
    preco->index_re_dkappaT_dmeff = index;
    index++;
    preco->index_re_orig_dkappa_dmeff = index;
    index++;
    preco->index_re_orig_dkappaT_dmeff = index;
    index++;
    preco->index_re_heat_chi_dmeff = index;
    index++;
    preco->index_re_heat_b_dmeff = index;
    index++;
    preco->index_re_cdmeff2 = index;
    index++;
  }
  ...
  /* dmeff quantities */
  if(pba->has_dmeff == _TRUE_){
    preio->index_re_Tdmeff = index;
    index++;
    preio->index_re_dkappa_dmeff = index;
    index++;
    preio->index_re_dkappaT_dmeff = index;
    index++;
    preio->index_re_orig_dkappa_dmeff = index;
    index++;
    preio->index_re_orig_dkappaT_dmeff = index;
    index++;
    preio->index_re_heat_chi_dmeff = index;
    index++;
    preio->index_re_heat_b_dmeff = index;
    index++;
    preio->index_re_cdmeff2 = index;
    index++;
  }
...
int thermodynamics_onthespot_energy_injection(
  ...
  rho_cdm_today = pow(pba->H0/_Mpc_over_m_*_c_,2)*3/8./_PI_/_G_*(pba->Omega0_cdm+pba->Omega0_dmeff)*_c_*_c_; /* energy density in J/m^3 */
...
/**
 * This subroutine contains the reionization function \f$ X_e(z) \f$ and \f$ X_H(z) \f$
...
int thermodynamics_reionization_function(
  ...
  if ((pth->reio_parametrization == reio_camb) || (pth->reio_parametrization == reio_half_tanh)) {
    ...
    if (z > preio->reionization_parameters[preio->index_reio_start]) {
      ...
      *xH = preio->reionization_parameters[preio->index_reio_xe_before];
 ...
 * @param pvecback   Input: vector of background quantities (used as workspace: must be already allocated, with format long_info or larger, but does not need to be filled)
 * @return the error status
 */
...
int thermodynamics_reionization(
  ...
  if ((pth->reio_parametrization == reio_camb) || (pth->reio_parametrization == reio_half_tanh)) {
    ...
    if (pth->reio_z_or_tau == reio_z) {
      ...
      class_call(thermodynamics_reionization_sample(ppr,pba,pth,preco,preio,pdmeff,pvecback),
                 pth->error_message,
                 pth->error_message);
...
int thermodynamics_reionization_sample(
  ...
  /* for dmeff */
  double dTdz_dmeff, collision_b, collision_dmeff;
  double *pvecthermo;
  ...
  /** - --> get \f$ X_e \f$ */
  class_call(thermodynamics_reionization_function(z,pth,preio,&xe,&xH),
             pth->error_message,
             pth->error_message);

  reio_vector[preio->index_re_xe] = xe;
  reio_vector[preio->index_re_xH] = xH;
  ...
  /** - --> dmeff */
  if(pba->has_dmeff == _TRUE_){
    reio_vector[preio->index_re_Tdmeff]        = preco->recombination_table[i*preco->re_size+preco->index_re_Tdmeff];
    reio_vector[preio->index_re_dkappa_dmeff]  = preco->recombination_table[i*preco->re_size+preco->index_re_dkappa_dmeff];
    reio_vector[preio->index_re_dkappaT_dmeff] = preco->recombination_table[i*preco->re_size+preco->index_re_dkappaT_dmeff];
    reio_vector[preio->index_re_orig_dkappa_dmeff]  = preco->recombination_table[i*preco->re_size+preco->index_re_orig_dkappa_dmeff];
    reio_vector[preio->index_re_orig_dkappaT_dmeff] = preco->recombination_table[i*preco->re_size+preco->index_re_orig_dkappaT_dmeff];
    reio_vector[preio->index_re_heat_chi_dmeff]  = preco->recombination_table[i*preco->re_size+preco->index_re_heat_chi_dmeff];
    reio_vector[preio->index_re_heat_b_dmeff]  = preco->recombination_table[i*preco->re_size+preco->index_re_heat_b_dmeff];
    reio_vector[preio->index_re_cdmeff2]       = preco->recombination_table[i*preco->re_size+preco->index_re_cdmeff2];
  }
  ...
  while (z > 0.) {
    ...
    class_call(thermodynamics_reionization_function(z_next,pth,preio,&xe_next,&xH),
               pth->error_message,
               pth->error_message);
  ...
  for (i=preio->rt_size-1; i >0 ; i--) {
    ...
    if(pba->has_dmeff == _TRUE_){
      ...
      pvecthermo[pth->index_th_Tdmeff] = preio->reionization_table[i*preio->re_size+preio->index_re_Tdmeff];
      pvecthermo[pth->index_th_Tb]     = preio->reionization_table[i*preio->re_size+preio->index_re_Tb];
      pvecthermo[pth->index_th_xe]     = preio->reionization_table[i*preio->re_size+preio->index_re_xe];
      pvecthermo[pth->index_th_xH]     = preio->reionization_table[i*preio->re_size+preio->index_re_xH];
      class_call(thermodynamics_dmeff_rate(pba,pth,pvecback,pvecthermo,0.0,0.0),
                 pth->error_message,
                 pth->error_message);

      preio->reionization_table[(i-1)*preio->re_size+preio->index_re_dkappa_dmeff]  = pvecthermo[pth->index_th_dkappa_dmeff];
      preio->reionization_table[(i-1)*preio->re_size+preio->index_re_dkappaT_dmeff] = pvecthermo[pth->index_th_dkappaT_dmeff];
      preio->reionization_table[(i-1)*preio->re_size+preio->index_re_orig_dkappa_dmeff]  = pvecthermo[pth->index_th_orig_dkappa_dmeff];
      preio->reionization_table[(i-1)*preio->re_size+preio->index_re_orig_dkappaT_dmeff] = pvecthermo[pth->index_th_orig_dkappaT_dmeff];
      preio->reionization_table[(i-1)*preio->re_size+preio->index_re_heat_chi_dmeff] = pvecthermo[pth->index_th_heat_chi_dmeff];
      preio->reionization_table[(i-1)*preio->re_size+preio->index_re_heat_b_dmeff] = pvecthermo[pth->index_th_heat_b_dmeff];
      preio->reionization_table[(i-1)*preio->re_size+preio->index_re_cdmeff2]       = pvecthermo[pth->index_th_cdmeff2];

      dTdz_dmeff = 2./(1.+z)*preio->reionization_table[i*preio->re_size+preio->index_re_Tdmeff];

      collision_dmeff = 2.*pvecthermo[pth->index_th_dkappaT_dmeff] * (preio->reionization_table[i*preio->re_size+preio->index_re_Tb]- preio->reionization_table[i*preio->re_size+preio->index_re_Tdmeff]) + pvecthermo[pth->index_th_heat_chi_dmeff];
      collision_dmeff /= -pvecback[pba->index_bg_H]; // convert derivative from tau to z

      if(isnan(collision_dmeff)==0){
        dTdz_dmeff += collision_dmeff;
      }

      if(pth->dmeff_backreaction_on_baryons == _TRUE_){
        collision_b = 2. * mu/pba->m_dmeff * pvecback[pba->index_bg_rho_dmeff]/pvecback[pba->index_bg_rho_b] * pvecthermo[pth->index_th_dkappaT_dmeff] * (preio->reionization_table[i*preio->re_size+preio->index_re_Tdmeff]-preio->reionization_table[i*preio->re_size+preio->index_re_Tb]) + pvecthermo[pth->index_th_heat_b_dmeff];
        collision_b /= -pvecback[pba->index_bg_H]; // convert derivative from tau to z

        if(isnan(collision_b)==0 && isnan(collision_dmeff)==0){
          dTdz += collision_b;
        }
      }

      preio->reionization_table[(i-1)*preio->re_size+preio->index_re_Tdmeff] =
        preio->reionization_table[i*preio->re_size+preio->index_re_Tdmeff]-dTdz_dmeff*dz;
    }
    ...
    preio->reionization_table[(i-1)*preio->re_size+preio->index_re_Tb] =
      ...
      if(pba->has_dmeff == _TRUE_){
        if(preio->reionization_table[(i-1)*preio->re_size+preio->index_re_Tb] < 0)preio->reionization_table[(i-1)*preio->re_size+preio->index_re_Tb] = preio->reionization_table[(i-1)*preio->re_size+preio->index_re_Tdmeff];
        if(preio->reionization_table[(i-1)*preio->re_size+preio->index_re_Tdmeff] < 0)preio->reionization_table[(i-1)*preio->re_size+preio->index_re_Tdmeff] = preio->reionization_table[(i-1)*preio->re_size+preio->index_re_Tb];
        if(preio->reionization_table[(i-1)*preio->re_size+preio->index_re_Tdmeff] > preio->reionization_table[(i-1)*preio->re_size+preio->index_re_Tb]) preio->reionization_table[(i-1)*preio->re_size+preio->index_re_Tdmeff]=preio->reionization_table[(i-1)*preio->re_size+preio->index_re_Tb];
      }
...
int thermodynamics_recombination(
  ...
  if (pth->recombination==hyrec) {
    ...
    // class_test((pba->has_dmeff),
    //            pth->error_message,
    //            "DMeff interactions not coded in for hyrec. Code it or use recfast.");

    class_call(thermodynamics_recombination_with_hyrec(ppr,pba,pth,preco,pdmeff,pvecback),
               pth->error_message,
               pth->error_message);
  ...
  if (pth->recombination==recfast) {
    ...
    class_call(thermodynamics_recombination_with_recfast(ppr,pba,pth,preco,pdmeff,pvecback),
               pth->error_message,
               pth->error_message);
...
int thermodynamics_recombination_with_hyrec(
  ...
  double *xe_output, *Tm_output, *Tdmeff_output;
  int i,j,l,Nz,b;
  double z, xe, Tm, Hz, Tdmeff,mu;
  FILE *fA;
  FILE *fR;
  ...
  param.omh2 = (pba->Omega0_b+pba->Omega0_cdm+pba->Omega0_dmeff+pba->Omega0_ncdm_tot)*pba->h*pba->h;
  ...
  param.Tdmeff_initial = pth->Tdmeff_initial; //Value at z = 10000
  param.Vrms_initial = pth->Vrms_initial; //Value at z = 10000
  param.omdmeffh2 = pba->Omega0_dmeff*pba->h*pba->h;
  param.m_dmeff = pba->m_dmeff;
  param.sigma_dmeff = pba->sigma_dmeff;
  param.npow_dmeff = pba->npow_dmeff;
  param.z_rate_off_dmeff = pba->z_rate_off_dmeff; //VG: switch off dmeff rates below thiz z.
  param.Vrel_dmeff_rate = pba->Vrel_dmeff_rate;

  if(pth->dmeff_backreaction_on_baryons == _TRUE_){
    param.dmeff_backreaction_on_baryons = 1;
  }
  else{
    param.dmeff_backreaction_on_baryons = 0;
  }
  /** - Build effective rate tables */

  /* allocate contiguous memory zone */

  buf_size = (2*NTR+NTM+2*NTR*NTM+3*param.nz)*sizeof(double) + 2*NTM*sizeof(double*);
  ...
  Tdmeff_output = (double*)(Tm_output+param.nz);  //Not sure of the memory allocation but it seems to be working.
  // Vrms_output = (double*)(Tm_output+param.nz);  //Not sure of the memory allocation but it seems to be working.
  ...
  rec_build_history(&param, &rate_table, &twog_params, xe_output, Tm_output, Tdmeff_output);
  ...
  // for(i=0; i<param.nz ; i++){
  //   printf("Tm_output %e Tdmeff_output %e\n",Tm_output[i],Tdmeff_output[i]);
  // }
  ...
  for(i=0; i <Nz; i++) {
    ...
    */
    ...
    if(pba->has_dmeff == _TRUE_) {
      pvecthermo[pth->index_th_Tdmeff] = Tdmeff;
      pvecthermo[pth->index_th_Tb]     = Tm;
      pvecthermo[pth->index_th_xe]     = xe;
      pvecthermo[pth->index_th_xH]     = xe;
      // assume xe = x_H, since we cannot properly extract x_H and x_He individually from hyrec
      class_call(thermodynamics_dmeff_rate(pba,pth,pvecback,pvecthermo,0.0,0.0),
                 pth->error_message,
                 pth->error_message);

      *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_Tdmeff) = Tdmeff;
      *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_cdmeff2) = pvecthermo[pth->index_th_cdmeff2];
      *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_dkappa_dmeff) = pvecthermo[pth->index_th_dkappa_dmeff];
      *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_dkappaT_dmeff) = pvecthermo[pth->index_th_dkappaT_dmeff];
      *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_orig_dkappa_dmeff) = pvecthermo[pth->index_th_orig_dkappa_dmeff];
      *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_orig_dkappaT_dmeff) = pvecthermo[pth->index_th_orig_dkappaT_dmeff];
      *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_heat_chi_dmeff) = pvecthermo[pth->index_th_heat_chi_dmeff];
      *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_heat_b_dmeff) = pvecthermo[pth->index_th_heat_b_dmeff];
      dQdlnadmeff = calculate_dQdnla_baryons(&param, z, xe,  Tm, Tdmeff);
    }
    else{
      dQdlnadmeff = 0;
    }
    ...
    *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_xH)=xe;
    ...
    *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_cb2)
      = _k_B_ / ( _c_ * _c_ * _m_H_ ) * (1. + (1./_not4_ - 1.) * pth->YHe + xe * (1.-pth->YHe)) * Tm * (1. - rec_dTmdlna(xe, Tm, pba->T_cmb*(1.+z), Hz, param.fHe, param.nH0*pow((1+z),3)*1e-6, energy_injection_rate(&param,z),dQdlnadmeff) / Tm / 3.);
...
int thermodynamics_recombination_with_recfast(
  ...
  /* vector of variables to be integrated: x_H, x_He, Tmat, Tdmeff */
  double y[_RECFAST_INTEG_SIZE_],dy[_RECFAST_INTEG_SIZE_];
  ...
  y[2] = preco->Tnow*(1.+z);
  if(pba->has_dmeff == _TRUE_){
    y[3] = pvecback[pba->index_bg_Tdmeff];
  } else{
    y[3] = 0;
  }
  // printf("a %e y[3] %e\n",pvecback[pba->index_bg_Tdmeff], y[3]);
  ...
  for(i=0; i <Nz; i++) {
    ...
    *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_xH)=y[0];
    ...
    *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_dkappadtau)
    ...
    /* dmeff */
    if(pba->has_dmeff == _TRUE_) {
      pvecthermo[pth->index_th_Tdmeff] = y[3];
      pvecthermo[pth->index_th_Tb]     = y[2];
      pvecthermo[pth->index_th_xe]     = x0;
      pvecthermo[pth->index_th_xH]     = y[0];
      class_call(thermodynamics_dmeff_rate(pba,pth,pvecback,pvecthermo,0.0,0.0),
                 pth->error_message,
                 pth->error_message);
      *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_Tdmeff) = y[3];
      *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_cdmeff2) = pvecthermo[pth->index_th_cdmeff2];
      *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_dkappa_dmeff) = pvecthermo[pth->index_th_dkappa_dmeff];
      *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_dkappaT_dmeff) = pvecthermo[pth->index_th_dkappaT_dmeff];
      *(preco->recombination_table+(Nz-i-1)*preco->re_size+preco->index_re_orig_dkappa_dmeff) = pvecthermo[pth->index_th_orig_dkappa_dmeff];
 ...
 *
 * Computes derivatives of the four variables to integrate: \f$ d x_H
 * / dz, d x_{He} / dz, d T_{mat} / dz, d T_{dmeff} / dz \f$.
 *
...
int thermodynamics_derivs_with_recfast(
  ...
  double Tdmeff, mu;
  double collision_dmeff, collision_b;
  double z_bug = 5;
  ...
  if(y[2] < 0) y[2] = y[3];//check first that y[2] isnt < 0
  if(y[3] < 0 || y[3] > y[2]) y[3] = y[2];
  ...
  // Rup = Rdown * pow((preco->CR*Trad),1.5)*exp(-preco->CDB/Trad);
  ...
  // Rup_He = 4.*Rdown_He*pow((preco->CR*Trad),1.5)*exp(-preco->CDB_He/Trad);
  K = preco->CK/Hz;
  ...
  // Rup_trip = Rdown_trip*exp(-_h_P_*_c_*_L_He2St_ion_/(_k_B_*Trad))*pow(preco->CR*Trad,1.5)*4./3.;
  ...
  if (timeTh < preco->H_frac*timeH) {
    // printf("z %e\n", z);
    // if(z<870 && z > 840){
      // printf("5z %e Tb %e dTb %e Tdmeff %e dTmeff %e \n", z,y[2],dy[2], Tdmeff,dy[3]);
    // }
    /*   dy[2]=Tmat/(1.+z); */
    ...
    // printf("afte z %e\n",z);
    // if(z<870 && z > 840){
      // printf("6 z %e Tb %e dTb %e Tdmeff %e dTmeff %e \n", z,y[2],dy[2], Tdmeff,dy[3]);
    // }
    /* equations modified to take into account energy injection from dark matter */
  ...
  if(pba->has_dmeff == _TRUE_  && 1+z > z_bug){
    //tend to crash below z = 5, but will be overwritten anyway by reionisation module.
    pvecthermo[pth->index_th_Tb] = Tmat;
    pvecthermo[pth->index_th_Tdmeff] = Tdmeff;
    pvecthermo[pth->index_th_xe] = x;
    pvecthermo[pth->index_th_xH] = x_H;
    class_call(thermodynamics_dmeff_rate(pba,pth,pvecback,pvecthermo,0.0,0.0),
               pth->error_message,
               pth->error_message);

    dy[3] = 2.*pvecback[pba->index_bg_a]*Tdmeff;

    collision_dmeff = 2.*pvecthermo[pth->index_th_dkappaT_dmeff] * (Tmat - Tdmeff) + pvecthermo[pth->index_th_heat_chi_dmeff];
    collision_dmeff /= -pvecback[pba->index_bg_H]; // convert derivative from tau to z

    if(isnan(collision_dmeff)==0){
      dy[3] += collision_dmeff;
    }

    if(pth->dmeff_backreaction_on_baryons == _TRUE_){
      if(timeTh < preco->H_frac*timeH){
        //   // In that regime, the equations are assuming Tm ~ Tgamma - Epsilon. This causes problem when DM-b scattering is on.
        //   // So even if we want to calculate the effect of DM-b scattering onto baryons, we do nothing.
        //   // Checked to be valid by comparing with the solution of the full temperature evolution with DM-b interactions.
        //   // You can check it too by setting preco->H_frac = 0 and recfast_Nz0 = 2000000.
        //   //UPDATE: by looking at 0902.3438 I believe that we can do slightly better by adding this expansion to our rate. In practice nothing changes though...
      }
      else{
        mu = _m_H_/(1. + (1./_not4_ - 1.) * pth->YHe + x * (1.-pth->YHe));
        collision_b = 2. * mu/pba->m_dmeff * pvecback[pba->index_bg_rho_dmeff]/pvecback[pba->index_bg_rho_b] * pvecthermo[pth->index_th_dkappaT_dmeff] * (Tdmeff - Tmat) + pvecthermo[pth->index_th_heat_b_dmeff];
        collision_b /= -pvecback[pba->index_bg_H]; // convert derivative from tau to z

        if(isnan(collision_b)==0 && isnan(collision_dmeff)==0 && fabs((y[2]-y[3])/y[3])<0.1){
          dy[2] += collision_b;
        }
      }
    }
...
int thermodynamics_merge_reco_and_reio(
  ...
  for (i=0; i < preio->rt_size; i++) {
    ...
    pth->thermodynamics_table[i*pth->th_size+pth->index_th_xH]=
      preio->reionization_table[i*preio->re_size+preio->index_re_xH];
    ...
    if(pba->has_dmeff == _TRUE_){
      pth->thermodynamics_table[i*pth->th_size+pth->index_th_Tdmeff]=
        preio->reionization_table[i*preio->re_size+preio->index_re_Tdmeff];
      pth->thermodynamics_table[i*pth->th_size+pth->index_th_dkappa_dmeff]=
        preio->reionization_table[i*preio->re_size+preio->index_re_dkappa_dmeff];
      pth->thermodynamics_table[i*pth->th_size+pth->index_th_dkappaT_dmeff]=
        preio->reionization_table[i*preio->re_size+preio->index_re_dkappaT_dmeff];
      pth->thermodynamics_table[i*pth->th_size+pth->index_th_orig_dkappa_dmeff]=
        preio->reionization_table[i*preio->re_size+preio->index_re_orig_dkappa_dmeff];
      pth->thermodynamics_table[i*pth->th_size+pth->index_th_orig_dkappaT_dmeff]=
        preio->reionization_table[i*preio->re_size+preio->index_re_orig_dkappaT_dmeff];
      pth->thermodynamics_table[i*pth->th_size+pth->index_th_heat_chi_dmeff]=
        preio->reionization_table[i*preio->re_size+preio->index_re_heat_chi_dmeff];
      pth->thermodynamics_table[i*pth->th_size+pth->index_th_heat_b_dmeff]=
        preio->reionization_table[i*preio->re_size+preio->index_re_heat_b_dmeff];
      pth->thermodynamics_table[i*pth->th_size+pth->index_th_cdmeff2]=
        preio->reionization_table[i*preio->re_size+preio->index_re_cdmeff2];
    }
  }
  for (i=0; i < ppr->recfast_Nz0 - preio->index_reco_when_reio_start - 1; i++) {
    ...
    pth->thermodynamics_table[index_th*pth->th_size+pth->index_th_xH]=
      preco->recombination_table[index_re*preco->re_size+preco->index_re_xH];
    ...
    if(pba->has_dmeff == _TRUE_){
      pth->thermodynamics_table[index_th*pth->th_size+pth->index_th_Tdmeff]=
        preco->recombination_table[index_re*preco->re_size+preco->index_re_Tdmeff];
      pth->thermodynamics_table[index_th*pth->th_size+pth->index_th_dkappa_dmeff]=
        preco->recombination_table[index_re*preco->re_size+preco->index_re_dkappa_dmeff];
      pth->thermodynamics_table[index_th*pth->th_size+pth->index_th_dkappaT_dmeff]=
        preco->recombination_table[index_re*preco->re_size+preco->index_re_dkappaT_dmeff];
      pth->thermodynamics_table[index_th*pth->th_size+pth->index_th_orig_dkappa_dmeff]=
        preco->recombination_table[index_re*preco->re_size+preco->index_re_orig_dkappa_dmeff];
      pth->thermodynamics_table[index_th*pth->th_size+pth->index_th_orig_dkappaT_dmeff]=
        preco->recombination_table[index_re*preco->re_size+preco->index_re_orig_dkappaT_dmeff];
      pth->thermodynamics_table[index_th*pth->th_size+pth->index_th_heat_chi_dmeff]=
        preco->recombination_table[index_re*preco->re_size+preco->index_re_heat_chi_dmeff];
      pth->thermodynamics_table[index_th*pth->th_size+pth->index_th_heat_b_dmeff]=
        preco->recombination_table[index_re*preco->re_size+preco->index_re_heat_b_dmeff];
      pth->thermodynamics_table[index_th*pth->th_size+pth->index_th_cdmeff2]=
        preco->recombination_table[index_re*preco->re_size+preco->index_re_cdmeff2];
...
int thermodynamics_output_titles(struct background * pba,
  ...
  class_store_columntitle(titles,"x_H",_TRUE_);
  ...
  class_store_columntitle(titles,"rate_orig_dmeff_mom [Mpc^-1]",pba->has_dmeff);
  class_store_columntitle(titles,"rate_orig_dmeff_temp [Mpc^-1]",pba->has_dmeff);
...
int thermodynamics_output_data(struct background * pba,
  ...
  for (index_z=0; index_z<pth->tt_size; index_z++){
    ...
    class_store_double(dataptr,pvecthermo[pth->index_th_xH],_TRUE_,storeidx);
    ...
    class_store_double(dataptr,pvecthermo[pth->index_th_orig_dkappa_dmeff],pba->has_dmeff,storeidx);
    class_store_double(dataptr,pvecthermo[pth->index_th_orig_dkappaT_dmeff],pba->has_dmeff,storeidx);
    class_store_double(dataptr,pvecthermo[pth->index_th_heat_chi_dmeff],pba->has_dmeff,storeidx);
    class_store_double(dataptr,pvecthermo[pth->index_th_heat_b_dmeff],pba->has_dmeff,storeidx);
...
/**
 *  This function initializes indices and allocates memory for dmeff.
 *
 * @param pba Input: background structure
 * @param pth Input: thermodynamics structure
 * @param pdmeff Output: dmeff_thermo structure
 */
...
int thermodynamics_dmeff_init(struct background *pba,
  ...
  /* Check for YHe. */
  class_test(pth->YHe == _BBN_,
             pth->error_message,"DMeff: Determine helium fraction YHe before initializing dmeff.");

  /* Define indices for dmeff temperature integration. tau must be last. */
  index_ti = 0;
  class_define_index(pdmeff->index_ti_Tdm,_TRUE_,index_ti,1);
  class_define_index(pdmeff->index_ti_tau,_TRUE_,index_ti,1);
  pdmeff->ti_size = index_ti;

  return _SUCCESS_;
}

/**
 *  This function frees dmeff memory.
 *
 * @param pdmeff Input/Output: dmeff_thermo structure
 */

int thermodynamics_dmeff_free(struct dmeff_thermo *pdmeff){
...
/**
 *  This function calculates the rates (in 1/Mpc, the same as Hubble) associated
 *  with momentum exchange (dkappa) and temperature exchange (dkappaT) during
 *  dmeff-baryon scattering. These rates are saved in pvecthermo.
 *
 *  The output is saved in pvecthermo entries:
 *      pth->index_th_dkappa_dmeff
 *      pth->index_th_dkappaT_dmeff
 *      pth->index_th_orig_dkappa_dmeff
 *      pth->index_th_orig_dkappaT_dmeff
 *      pth->index_th_heat_chi_dmeff
 *      pth->index_th_heat_b_dmeff
 ...
 *  Must input H ionization fraction via pvecthermo[pth->index_th_xH].
 *
 * @param pba        Input:        background structure
 * @param pth        Input:        thermodynamics structure
 * @param pvecback   Input:        background vector as workspace
 * @param pvecthermo Input/Output: thermodynamics vector as workspace
 * @param Vrms       Input:        RMS velocity (relevant for calling within perturbations)
 * @param Vflow      Input:        flow velocity  (relevant for calling within perturbations)
 *
 */
...
int thermodynamics_dmeff_rate(struct background *pba,
  ...
  double z, a, H, rho_baryon, nH, mu, YHe;
  double masses[2],rho_species[2];
  double Tb, Tdmeff, dTdtau_T;
  double cn, Vrel2, vth2, rate_mom, rate_heat;
  double F1_mom, F1_temp_5_2, F1_temp_3_2;
  int last_index;
  int i, i_max;
  ...
  if(pba->sigma_dmeff == 0.0){
    ...
    pvecthermo[pth->index_th_orig_dkappa_dmeff] = 0.0;
    pvecthermo[pth->index_th_orig_dkappaT_dmeff] = 0.0;
    pvecthermo[pth->index_th_heat_chi_dmeff] = 0.0;
    pvecthermo[pth->index_th_heat_b_dmeff] = 0.0;
  ...
  z = 1./a - 1.;
  if(pth->Tracy_and_Cora_Helium == _TRUE_){
    YHe = 0.24;
  } else{
    YHe = pth->YHe;
  }
  ...
  nH = rho_baryon/_m_H_ * (1.-YHe);
  mu = _m_H_/(1. + (1./_not4_ - 1.) * YHe + pvecthermo[pth->index_th_xe] * (1.-YHe)); //assume fully ionize universe

  /* set info for various baryonic species that interact with DM */
  if(pth->dmeff_interacting_species == ionized){
    // DM interacts with free protons and electrons, but not ionized helium
    i_max = 2;
    masses[0]=_m_p_;
    masses[1]=_m_e_;
    rho_species[0] = _m_p_ * nH * pvecthermo[pth->index_th_xH];
    rho_species[1] = _m_e_ * nH * pvecthermo[pth->index_th_xe];
  }
  else if (pth->dmeff_interacting_species == hydrogen){
    // DM interacts with protons, whether in neutral or ionized hydrogen, but not helium
    i_max = 1;
    masses[0] = _m_H_;
    rho_species[0] = (1.-YHe)*rho_baryon;
  }
  else if (pth->dmeff_interacting_species == baryons){
    // DM interacts with all baryons
    i_max = 1;
    masses[0] = mu;
    rho_species[0] = rho_baryon;
  }
  ...
  /* Initialize rates to save in pvecthermo. */
  pvecthermo[pth->index_th_dkappa_dmeff]  = 0.0;
  pvecthermo[pth->index_th_dkappaT_dmeff] = 0.0;
  pvecthermo[pth->index_th_orig_dkappa_dmeff]  = 0.0;
  pvecthermo[pth->index_th_orig_dkappaT_dmeff] = 0.0;
  ...
  Vrel2 = pow(pvecback[pba->index_bg_Vrms],2);

  for(i=0; i<i_max; i++){
    /* thermal dispersion */
    if(pth->calculation_a_la_dvorkin_et_al == _TRUE_ || pth->sigma_dmeff_security == _TRUE_){
      vth2 = (Tdmeff*_k_B_) / pba->m_dmeff + (Tb*_k_B_) / masses[i] + Vrel2/3.;
    } else{
      vth2 = (Tdmeff*_k_B_) / pba->m_dmeff + (Tb*_k_B_) / masses[i];
    }

    /* original rates, with no contributions from relative bulk velocity (unless running a_la_dvorkin) */
    rate_mom  = a * rho_species[i] * cn * pba->sigma_dmeff/(pba->m_dmeff + masses[i]) * pow(vth2/(_c_*_c_), (pba->npow_dmeff + 1.)/2.);
    rate_heat = rate_mom * pba->m_dmeff / (pba->m_dmeff + masses[i]);

    /* save rates */
    pvecthermo[pth->index_th_orig_dkappa_dmeff]  += rate_mom;
    pvecthermo[pth->index_th_orig_dkappaT_dmeff] += rate_heat;

    /* handle modified rates */
    if(pth->calculation_a_la_dvorkin_et_al == _TRUE_ || pth->sigma_dmeff_security == _TRUE_){
      // rates are already modified via vth2; do not include heating from Vrel dissipation
      pvecthermo[pth->index_th_dkappa_dmeff]  += rate_mom;
      pvecthermo[pth->index_th_dkappaT_dmeff] += rate_heat;
    } else{
      /* set hypergeometric functions */
      F1_mom = gsl_sf_hyperg_1F1(-(pba->npow_dmeff+1.)/2.,5./2.,-(Vflow*Vflow)/(2.*(vth2 + Vrms*Vrms/3.)));
      F1_temp_5_2 = gsl_sf_hyperg_1F1(-(pba->npow_dmeff+1.)/2.,5./2.,-Vrel2/(2.*vth2));
      F1_temp_3_2 = gsl_sf_hyperg_1F1(-(pba->npow_dmeff+3.)/2.,3./2.,-Vrel2/(2.*vth2));

      /* modified momentum transfer -- assumes form of full k-dependent treatment,
         but it is ultimately controlled by the choice of inputs Vrms and Vflow */
      pvecthermo[pth->index_th_dkappa_dmeff]  += rate_mom * pow(1. + (Vrms*Vrms)/(3.*vth2),(pba->npow_dmeff+1.)/2.)*F1_mom;

      /* modified heat transfer */
      if(pth->dmeff_baryon_heat_exchange == _TRUE_){
        pvecthermo[pth->index_th_dkappaT_dmeff] += rate_heat *  (F1_temp_3_2 - Vrel2/(3.*vth2) * F1_temp_5_2);
        pvecthermo[pth->index_th_heat_chi_dmeff] += (2./3.) * rate_heat * masses[i] * Vrel2 * F1_temp_5_2 / _k_B_;
        if(pth->dmeff_backreaction_on_baryons == _TRUE_){
          pvecthermo[pth->index_th_heat_b_dmeff] += (2./3.) * rate_heat * mu * pvecback[pba->index_bg_rho_dmeff]/pvecback[pba->index_bg_rho_b] * Vrel2 * F1_temp_5_2 / _k_B_;
        }
      }
    }
  ...
  /* Artificially turn off interaction. */
  if((z<pba->z_rate_off_dmeff) || (z>pba->z_rate_on_dmeff)){
    pvecthermo[pth->index_th_dkappa_dmeff] = 0.0;
    pvecthermo[pth->index_th_dkappaT_dmeff] = 0.0;
    pvecthermo[pth->index_th_orig_dkappa_dmeff] = 0.0;
    pvecthermo[pth->index_th_orig_dkappaT_dmeff] = 0.0;
  ...
  /* Also fill in c^2 for dmeff. Set dTdtau_T = (dTdmeff / dtau)/Tdmeff appropriately.
     Full expression for dTdmeff/dtau is from thermodynamics_dmeff_derivs(). */
  dTdtau_T = -2.*a*H + 2.*pvecthermo[pth->index_th_dkappaT_dmeff] * (Tb/Tdmeff - 1.0);
  pvecthermo[pth->index_th_cdmeff2] = Tdmeff*_k_B_/(pba->m_dmeff*_c_*_c_) * (1. - dTdtau_T/(3.*a*H));
...
int thermodynamics_dmeff_derivs(double tau,
  ...
  struct dmeff_parameters_and_workspace * pdmpaw;
  struct dmeff_thermo * pdmeff;
  struct background * pba;
  struct thermo *pth;
  double * pvecback;
  double * pvecthermo;
  double a,H,z,collision;
  int last_index;

  pdmpaw = parameters_and_workspace;
  pba  = pdmpaw->pba;
  pth  = pdmpaw->pth;
  pdmeff = pdmpaw->pdmeff;
  pvecback   = pdmpaw->pvecback;
  pvecthermo = pdmpaw->pvecthermo;
  ...
  /* Find energy exchange rate. Fix baryon temperature to photon temperature. */
  pvecthermo[pth->index_th_Tdmeff] = y[pdmeff->index_ti_Tdm];
  pvecthermo[pth->index_th_Tb]     = pba->T_cmb * (1.+z);
  pvecthermo[pth->index_th_xe]     = 1.0 + 2.*pth->YHe/(_not4_ *(1.-pth->YHe)); // assume fully ionized universe
  pvecthermo[pth->index_th_xH]     = 1.0; // assume fully ionized universe
  ...
  if (z > pdmeff->zdec){
    dy[pdmeff->index_ti_Tdm] = -a*H*y[pdmeff->index_ti_Tdm];
  } else {

    dy[pdmeff->index_ti_Tdm] = -2.*a*H*y[pdmeff->index_ti_Tdm];

    collision = 2.*pvecthermo[pth->index_th_dkappaT_dmeff] * (pvecthermo[pth->index_th_Tb] - y[pdmeff->index_ti_Tdm]) + pvecthermo[pth->index_th_heat_chi_dmeff];

    if(isnan(collision)==0){
      dy[pdmeff->index_ti_Tdm] += collision; // include effects of scattering if rate is not nan
    }
  }
...
int thermodynamics_dmeff_temperature(struct precision *ppr,
  ...
  struct dmeff_parameters_and_workspace dmpaw;
  double * pvecback;
  double * pvecthermo;
  ...
  int tt_size_check;
  double *d2bg_dtau2_table;

  /* Allocate memory. */
  class_alloc(pvecback,  pba->bg_size*sizeof(double),pth->error_message);
  class_alloc(pvecthermo,pth->th_size*sizeof(double),pth->error_message);
  class_alloc(pvecdmeff_integration,pdmeff->ti_size*sizeof(double),pth->error_message);

  dmpaw.pba = pba;
  dmpaw.pth = pth;
  dmpaw.pdmeff = pdmeff;
  dmpaw.pvecback = pvecback;
  dmpaw.pvecthermo = pvecthermo;

  /* Initialize integrator (tau is not integrated). */
  class_call(initialize_generic_integrator((pdmeff->ti_size-1),&gi),
             gi.error_message,
             pth->error_message);
  class_call(gt_init(&gTable),
             gTable.error_message,
  ...
  pvecdmeff_integration[pdmeff->index_ti_Tdm] = pba->T_cmb / a;
  pvecdmeff_integration[pdmeff->index_ti_tau] = pba->tau_table[0];
  tau_end = pvecdmeff_integration[pdmeff->index_ti_tau];
  tt_size_check = 0;
  ...
  for(index_tau=0; index_tau < pba->bt_size-1; index_tau++){
    ...
    class_call(gt_add(&gTable,_GT_END_,(void *) pvecdmeff_integration,sizeof(double)*pdmeff->ti_size),
               gTable.error_message,
               pth->error_message);
    tt_size_check++;
    // printf("a before %e pba->index_bg_a %d pba->bg_size %d bg_size_normal %d\n", pvecback[pba->index_bg_a],pba->index_bg_a,pba->bg_size,pba->bg_size_normal);
    /* -> perform one step */
    ...
    // printf("a after %e pba->index_bg_a %d\n", pvecback[pba->index_bg_a],pba->index_bg_a);

    /* -> store value of tau */
    pvecdmeff_integration[pdmeff->index_ti_tau]=tau_end;

    /* -> Test when Hubble rate surpasses 1/10 of the rate of dmeff interactions,
       and set decoupling to this value. **/
    if(pdmeff->zdec < 0.0){
      a = pvecback[pba->index_bg_a];
      H = pvecback[pba->index_bg_H];
      if(a*H > pvecthermo[pth->index_th_orig_dkappaT_dmeff]/100.){
        pdmeff->zdec = 1./a - 1.;
      }
    }
  ...
  for (index_tau=0; index_tau < pba->bt_size; index_tau++) {
    ...
    pvecthermo[pth->index_th_Tdmeff] = pData[index_tau*pdmeff->ti_size+pdmeff->index_ti_Tdm];
    pvecthermo[pth->index_th_Tb]     = pba->T_cmb / pvecback[pba->index_bg_a];
    pvecthermo[pth->index_th_xe]     = 1+2.*pth->YHe/(_not4_ *(1.-pth->YHe)); // assume fully ionized universe
    pvecthermo[pth->index_th_xH]     = 1.0; // assume fully ionized universe
    class_call(thermodynamics_dmeff_rate(pba,pth,pvecback,pvecthermo,0.0,0.0),
               pth->error_message,
               pth->error_message);
    ...
    // entry for index_bg_Vrms already initialized in background module
    pba->background_table[index_tau*pba->bg_size+pba->index_bg_dkappa_dmeff] =
      pvecthermo[pth->index_th_dkappa_dmeff];
    pba->background_table[index_tau*pba->bg_size+pba->index_bg_dkappaT_dmeff] =
      pvecthermo[pth->index_th_dkappaT_dmeff];
    pba->background_table[index_tau*pba->bg_size+pba->index_bg_orig_dkappa_dmeff] =
      pvecthermo[pth->index_th_orig_dkappa_dmeff];
    pba->background_table[index_tau*pba->bg_size+pba->index_bg_orig_dkappaT_dmeff] =
      pvecthermo[pth->index_th_orig_dkappaT_dmeff];
    pba->background_table[index_tau*pba->bg_size+pba->index_bg_heat_chi_dmeff] =
      pvecthermo[pth->index_th_heat_chi_dmeff];
    pba->background_table[index_tau*pba->bg_size+pba->index_bg_heat_b_dmeff] =
      pvecthermo[pth->index_th_heat_b_dmeff];
    pba->background_table[index_tau*pba->bg_size+pba->index_bg_cdmeff2] =
      ...
      if( z > ppr->recfast_z_initial){
        pdmeff->Tdmeff_initial = pvecthermo[pth->index_th_Tdmeff]; //Get Tdmeff_initial for HyRec
        pdmeff->Vrms_initial = pba->Vrel_dmeff_rate; //Get Tdmeff_initial for HyRec
      }
  }

  /** - create table of derivatives  */
  class_alloc(d2bg_dtau2_table,pba->bt_size * pba->bg_size * sizeof(double),pth->error_message);
  ...
  for (index_tau=0; index_tau < pba->bt_size; index_tau++) {
    pba->d2background_dtau2_table[index_tau*pba->bg_size+pba->index_bg_Tdmeff] =
      d2bg_dtau2_table[index_tau*pba->bg_size+pba->index_bg_Tdmeff];
    pba->d2background_dtau2_table[index_tau*pba->bg_size+pba->index_bg_Vrms] =
      d2bg_dtau2_table[index_tau*pba->bg_size+pba->index_bg_Vrms];
    pba->d2background_dtau2_table[index_tau*pba->bg_size+pba->index_bg_dkappa_dmeff] =
      d2bg_dtau2_table[index_tau*pba->bg_size+pba->index_bg_dkappa_dmeff];
    pba->d2background_dtau2_table[index_tau*pba->bg_size+pba->index_bg_dkappaT_dmeff] =
      d2bg_dtau2_table[index_tau*pba->bg_size+pba->index_bg_dkappaT_dmeff];
    pba->d2background_dtau2_table[index_tau*pba->bg_size+pba->index_bg_orig_dkappa_dmeff] =
      d2bg_dtau2_table[index_tau*pba->bg_size+pba->index_bg_orig_dkappa_dmeff];
    pba->d2background_dtau2_table[index_tau*pba->bg_size+pba->index_bg_orig_dkappaT_dmeff] =
      d2bg_dtau2_table[index_tau*pba->bg_size+pba->index_bg_orig_dkappaT_dmeff];
    pba->d2background_dtau2_table[index_tau*pba->bg_size+pba->index_bg_heat_chi_dmeff] =
      d2bg_dtau2_table[index_tau*pba->bg_size+pba->index_bg_heat_chi_dmeff];
    pba->d2background_dtau2_table[index_tau*pba->bg_size+pba->index_bg_heat_b_dmeff] =
      d2bg_dtau2_table[index_tau*pba->bg_size+pba->index_bg_heat_b_dmeff];
    pba->d2background_dtau2_table[index_tau*pba->bg_size+pba->index_bg_cdmeff2] =
      d2bg_dtau2_table[index_tau*pba->bg_size+pba->index_bg_cdmeff2];
