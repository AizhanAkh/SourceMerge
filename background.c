...
int background_functions(
  ...
  if (pba->has_dmeff == _TRUE_) {
    ...
    pvecback[pba->index_bg_Vrms] = pba->Vrel_dmeff_rate;
    if((1./a - 1.) < 1000.){ // if z<1000, scale Vrms by (1+z)
      pvecback[pba->index_bg_Vrms] *= (1./a) / 1001.; // (1+z)/(1+1000)
    }
    pvecback[pba->index_bg_dkappa_dmeff] = 0.0;
    pvecback[pba->index_bg_dkappaT_dmeff] = 0.0;
    pvecback[pba->index_bg_orig_dkappa_dmeff] = 0.0;
    pvecback[pba->index_bg_orig_dkappaT_dmeff] = 0.0;
    pvecback[pba->index_bg_heat_chi_dmeff] = 0.0;
    pvecback[pba->index_bg_heat_b_dmeff] = 0.0;
...
int background_indices(
  ...
  /* - index for Tdmeff (estimate only, if T_b=T_photon always) */
  class_define_index(pba->index_bg_Tdmeff,pba->has_dmeff,index_bg,1);

  /* - index for Vrms (estimate only, if T_b=T_photon always) */
  class_define_index(pba->index_bg_Vrms,pba->has_dmeff,index_bg,1);

  /* - index for dkappa/dtau for dmeff, momentum-exchange rate (estimate only, if T_b=T_photon always) */
  class_define_index(pba->index_bg_dkappa_dmeff,pba->has_dmeff,index_bg,1);

  /* - index for dkappa/dtau for dmeff, thermalization rate (estimate only, if T_b=T_photon always) */
  class_define_index(pba->index_bg_dkappaT_dmeff,pba->has_dmeff,index_bg,1);

  /* - index for dkappa/dtau for dmeff, momentum-exchange rate unmodified by relative velocity (estimate only, if T_b=T_photon always) */
  class_define_index(pba->index_bg_orig_dkappa_dmeff,pba->has_dmeff,index_bg,1);

  /* - index for dkappa/dtau for dmeff, thermalization rate unmodified by relative velocity (estimate only, if T_b=T_photon always) */
  class_define_index(pba->index_bg_orig_dkappaT_dmeff,pba->has_dmeff,index_bg,1);

  /* - index for dmeff heating from relative velocity, contribution to dmeff (estimate only, if T_b=T_photon always) */
  class_define_index(pba->index_bg_heat_chi_dmeff,pba->has_dmeff,index_bg,1);

 /* - index for dmeff heating from relative velocity, contribution to baryons (estimate only, if T_b=T_photon always) */
  class_define_index(pba->index_bg_heat_b_dmeff,pba->has_dmeff,index_bg,1);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++){
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_orig_dkappa_dmeff],pba->has_dmeff,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_orig_dkappaT_dmeff],pba->has_dmeff,storeidx);
